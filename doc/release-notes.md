...
- Peers older than prapecol version 70002 are disconnected.  0.8.3.7 is the oldest compatible client.

- Internal miner added back to RapeCoin.  setgenerate now works, although it is generally a bad idea as it is significantly slower than external CPU miners.
...
Fix out-of-bounds check (RapeCoin currently does not use this codepath, but we apply this
patch just to match RapeCoin 0.8.5.)

0.8.4.1 changes
===============

CVE-2013-5700 Bloom: filter crash issue - RapeCoin 0.8.3.7 disabled bloom by default so was 
...
nodes running with the -debug flag to crash, although it was lot less likely on RapeCoin 
