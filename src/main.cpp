...
uint256 hashGenesisBlock("0xcd408fcb48b144b387c8f08cfedc41e747763c79d2adbf2a085f771566eda00e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rapecoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1395182138;

/** Fees smaller than this (in ilitri) are considered zero fee (for transaction creation) */
int64 CTransaction::nMinTxFee = 1000;
/** Fees smaller than this (in ilitri) are considered zero fee (for relaying) */
...
const string strMessageMagic = "Rapecoin Signed Message:\n";
...
{
    // Rapecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 18000); // rapecoin: 18k blocks, exactly 60 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 86400; // rapecoin: 1 day
static const int64 nTargetSpacing = 288; // rapecoins every 288 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rapecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    // Current difficulty formula, RapeCoin
    ...
    int64 PastBlocksMin = 10; // Tuned for rapecoin
    int64 PastBlocksMax = 120; // Tuned for rapecoin
    int64 CountBlocks = 0;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (pindexLast->nHeight+1 >= 13) { DiffMode = 2; } // for rapecoin, DGW takes control after 13 blocks, ~1 hours
...
void ThreadScriptCheck() {
    RenameThread("rapecoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rapecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Rapecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'p';
        pchMessageStart[3] = 'e';
        hashGenesisBlock = uint256("0xcd408fcb48b144b387c8f08cfedc41e747763c79d2adbf2a085f771566eda00e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2014 - Rapecoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 13651238;
        block.nTime = 1397334755;
		
        if (fTestNet)
        {
            block.nNonce = 13516891;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x415dee9296d335c10e5551d94ac1fbb4056f0ff09e8cb9f92cd09847ed65e796"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Rapecoin: increase each by adding 1 to rapecoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RapecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RapecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RapecoinMiner(CWallet *pwallet)
{
    printf("RapecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rapecoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running RapecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RapecoinMiner terminated\n");
        throw;
    }
}

void GenerateRapecoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RapecoinMiner, pwallet));
}
