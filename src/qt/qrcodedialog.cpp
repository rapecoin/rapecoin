...
{
    QString ret = QString("rapecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non Rap unit input in lnReqAmount, we generate the URI with Rap as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(RapecoinUnits::format(RapecoinUnits::Rap, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
